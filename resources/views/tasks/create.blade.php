        ...
        <section class="grow mt-24 flex flex-col px-52">
            ...
            <div class="w-115 mt-4 mx-auto flex flex-col items-center">
                ...
                <form action="{{route('task.store')}}" method="post" novalidate class="w-full">
                    @csrf
                    <x-form.input type="text" name="title_en" placeholder="" label="{{__('tasks.task_name_en')}}"  value="{{old('title_en')}}"/>
                    <x-form.input type="text" name="title_ka" placeholder=""  label="{{__('tasks.task_name_ka')}}" value="{{old('title_ka')}}"/>
                    
                    <x-form.textarea name="description_en" placeholder=""  label="{{__('tasks.task_description_en')}}" value="{{old('description_en')}}" />
                    <x-form.textarea name="description_ka" placeholder=""  label="{{__('tasks.task_description_ka')}}" value="{{old('description_ka')}}" />
